...
import {JWKInterface} from "arbundles/build/web/cjs/src/interface-jwk"
import { Signer } from "arbundles/build/web/cjs/src/signing/Signer";
import cryptoDriver from "arweave/web/lib/crypto/webcrypto-driver"
import { jwkTopem } from "arweave/web/lib/crypto/pem"

const arweaveCrypto = new cryptoDriver()

const ownerLength = 512
const signatureLength = 512

export class Rsa4096Pss implements Signer {
...
//   private readonly _publicKey: Buffer;
  ...
  constructor(private _key: JWKInterface, public pk: string) {
    // if (!pk) {
    //   this.pk = crypto.getPublicKey(JSON.parse(_key));
    // }
  }
...
export class ArweaveSigner extends Rsa4096Pss {
    ...
    get publicKey(): Buffer {
     ...
     sign(message: Uint8Array): Uint8Array {
      return arweaveCrypto.sign(this.jwk, message) as any;
    }
  
    static async verify(pk: string, message: Uint8Array, signature: Uint8Array): Promise<boolean> {
      return await arweaveCrypto.verify(pk, message, signature);
    }
  }
...
export class DataItem {
    ...
    public async sign(signer: ArweaveSigner): Promise<Buffer> {
